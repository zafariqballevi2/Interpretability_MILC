...
class combinedModel(nn.Module):
    ...
    def forward(self, sx, mode="train"):
        ...
        #print("inputstolstm",inputs[0].shape, len(inputs))
        #print("bb", inputs[0].shape, len(inputs))
        #exit()
        #print("aaaa", inputs[0].shape)
        #print(self.lstm)
        outputs = self.lstm(inputs, mode)
        #print("ouuuuuu", outputs.shape)
        logits = self.get_attention(outputs)
        #print("logitsfromlstm", logits.shape)
        #exit()
