...
Dataset = {0: 'HCP', 1: 'COBRE', 2: 'OASIS', 3: 'ABIDE'}
gain = {'COBRE': [0.05, 0.65, 0.75], 'HCP': [0.85, 0.4, 0.35], 'ABIDE': [0.3, 0.35, 0.8],
        'OASIS': [0.4, 0.65, 0.35]}

Directories = { 'COBRE': 'COBRESaliencies','HCP' : 'FBIRNSaliencies', 'ABIDE' : 'ABIDESaliencies','OASIS' : 'OASISSaliencies'}

#FNCDict = {"FBIRN": LoadFBIRN, "COBRE": LoadCOBRE, "OASIS": LoadOASIS, "ABIDE": LoadABIDE}

Params = {'HCP':[790, 1040, 52], 'COBRE': [157, 140, 121], 'OASIS': [372, 120, 101], 'ABIDE': [569, 140, 121]}

train_lim = { 'HCP': 250, 'COBRE': 100, 'OASIS': 300, 'ABIDE': 400}

Params_subj_distr = { 'HCP': [790], 'COBRE': [15, 25, 40], 'OASIS': [15, 25, 50, 75, 100, 120], 'ABIDE': [15, 25, 50, 75, 100, 150]}
#[15, 25, 50, 75, 100]
test_lim_per_class = { 'HCP': 32, 'COBRE': 16, 'OASIS': 32, 'ABIDE': 50}

Params_best_gains = {1: {'HCP': {'NPT':[0.9], 'UFPT': [0.9, 0.7, 0.6, 0.5, 0.4]}, 'ABIDE': {'NPT': [1.3, 1.3, 1.1, 0.9, 1.3, 0.6], 'UFPT': [0.4, 0.6, 0.5, 0.5, 0.3, 1.2]}, 'OASIS': {'NPT': [0.1, 1.2, 1.0, 1.0, 1.2, 0.7], 'UFPT': [0.7, 0.7, 1.3, 0.6, 1.1, 1.2]},'COBRE': {'NPT': [0.9, 0.9, 1.0], 'UFPT': [0.4, 1.1, 1.3]}}, 20: {'COBRE': {'NPT': [0.6, 1.1, 1.1], 'UFPT': [1.1, 1.5, 0.6]}}}
#Params_best_gains = {1: {'HCP': {'NPT':[1.2, 1.3, 0.7, 1.1, 0.9], 'UFPT': [0.9, 0.7, 0.6, 0.5, 0.4]}, 'ABIDE': {'NPT': [1.3, 1.3, 1.1, 0.9, 1.3, 0.6], 'UFPT': [0.4, 0.6, 0.5, 0.5, 0.3, 1.2]}, 'OASIS': {'NPT': [0.1, 1.2, 1.0, 1.0, 1.2, 0.7], 'UFPT': [0.7, 0.7, 1.3, 0.6, 1.1, 1.2]},'COBRE': {'NPT': [0.9, 0.9, 1.0], 'UFPT': [0.4, 1.1, 1.3]}}, 20: {'COBRE': {'NPT': [0.6, 1.1, 1.1], 'UFPT': [1.1, 1.5, 0.6]}}}
############



path = Path('/data/users2/ziqbal5/MILC/Data/HCP.pickle')
...
tp=len(Data[0][0])
print(Data.shape)
#reversing time slices
ccc = np.empty([len(Data), 53, tp])
...
#combined data
C_data = np.concatenate((Data, ccc), axis = 0)
print("shape before windowing", C_data.shape)
...
data = 0
mode = 1
#data= int(sys.argv[1])  # Choose HCP (0)/ COBRE (1) / OASIS (2) / ABIDE (3)
...
no_good_comp = 53
sample_y = 20
subjects = len(Data)*2 #Params[Dataset[data]][0]
tc =  tp   #Params[Dataset[data]][1]
window_shift = 20
samples_per_subject = 52
#samples_per_subject = int((tp-sample_y)/window_shift) # Params[Dataset[data]][2]
...
test_starts = { 'HCP': [0, 32, 64, 96, 120], 'COBRE': [0, 16, 32, 48], 'OASIS': [0, 32, 64, 96, 120, 152], 'ABIDE': [0, 50, 100, 150, 200]}
...
Best_gain = Params_best_gains[1][Dataset[data]][MODELS[mode]]
...
#         model.to(device)
        optimizer, accMat[i, restart], aucMat[i, restart], model = train_model(model, dataLoaderTrain, dataLoaderTrainCheck, dataLoaderTest, eppochs, .0005)
        PATH = '/data/users2/ziqbal5/MILC_LSTM/pretrainedModels/'
        if args.encoder == 'cnn':
            torch.save(model.state_dict(),  os.path.join(PATH, str(args.jobid)+ '_' + str(restart)+ '_CNN.pt'))  
        elif args.encoder == 'lstm':
            torch.save(model.state_dict(),  os.path.join(PATH, str(args.jobid)+ '_' + str(restart)+ '_LSTM.pt'))
        elif args.encoder == 'lstmM':
            torch.save(model.state_dict(),  os.path.join(PATH, str(args.jobid)+ '_' + str(restart)+ '_LSTMM.pt'))
