...
def generate_sample_synthetic_data(samples, t_steps, features, p_steps, seed):
        ...
        with open(os.path.join(path_a, str(args.jobid) +'_' +str(dd) + 's_position.pickle'), "wb") as outfile:
            pickle.dump(start_positions, outfile)            

        return tr_data, tr_labels, test_data, test_labels,  start_positions, masks    

# check = Path('/data/users2/ziqbal5/abc/MILC_LSTM/Data_CNN/tr_data.pickle')
# if check.is_file():
#     with open(os.path.join(path_a, 'tr_data.pickle'), "rb") as infile:
#         tr_data = pickle.load(infile)
#     with open(os.path.join(path_a, 'tr_labels.pickle'), "rb") as infile:
#         tr_labels = pickle.load(infile)    
...
for dd in range(seeds):
    print("Seed: ", dd)
    tr_data, tr_labels, _, _, _,_ = generate_sample_synthetic_data(2500, 140, 53, 10, dd)
